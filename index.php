...
foreach (idea::find(idea::POPULAR, 25) as $idea)
{
	$template->assign_block_vars('ideas', array_merge(array(
		...
		'S_CAN_VOTE_UP'		=> ($idea->can_vote() || ($idea->can_vote_change() && $idea->get_vote()->value === vote::DOWN)),
		'S_CAN_VOTE_DOWN'	=> ($idea->can_vote() || ($idea->can_vote_change() && $idea->get_vote()->value === vote::UP)),
		...
		'U_VOTE_UP'		=> ($idea->can_vote() || ($idea->can_vote_change() && $idea->get_vote()->value === vote::DOWN)) ? append_sid(NP_ROOT_PATH . '/vote.' . $phpEx, "i={$idea->id}&amp;up&amp;place") : false,
		'U_VOTE_DOWN'	=> ($idea->can_vote() || ($idea->can_vote_change() && $idea->get_vote()->value === vote::UP)) ? append_sid(NP_ROOT_PATH . '/vote.' . $phpEx, "i={$idea->id}&amp;down&amp;place") : false,
